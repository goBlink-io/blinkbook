...
function renderList(node: TiptapNode, type: 'bullet' | 'ordered', depth = 0): string {
  if (!node.content) return '';
    ...
    .map((item, index) => {
      ...
      for (const child of item.content ?? []) {
        if (child.type === 'bulletList') {
          ...
          // First inline block becomes the list item text
          if (parts.length === 0) {
            parts.push(`${indent}${prefix} ${renderInlineContent(child.content)}`);
          } else {
